...
fn is_supported_scheme(scheme: &str) -> bool {
    matches!(scheme, "cle-console" | "cleconsole")
}
...
/// the binary name as the first element (e.g. `["cle-console",
/// "cle-console://import?..."]`).  Consuming it wastes a log line, can
/// mislead diagnostics (`is_deep_link=false, candidate='cle-console'`),
...
pub fn handle_external_import_args<R: Runtime>(
    ...
    for (i, arg) in args.iter().enumerate() {
        ...
        let candidate_is_deep_link =
            candidate.starts_with("cle-console://") || candidate.starts_with("cleconsole://");
...
mod tests {
    ...
    fn parse_basic_import_link() {
        let raw = "cle-console://import?provider=codex&token=abc123";
    ...
    fn parse_alias_and_boolean() {
        let raw =
            "cle-console://provider-import?platform=codebuddy-cn&payload=%7B%7D&auto_import=true";
        let payload = parse_external_import_url(raw).expect("payload");
    ...
    fn parse_antigravity_overview_alias() {
        let raw = "cleconsole://account-import?page=overview&token=1%2F%2F0gTokenDemo";
        let payload = parse_external_import_url(raw).expect("payload");
    ...
    fn parse_import_url_link() {
        let raw = "cle-console://import?provider=codex&import_url=https%3A%2F%2Fexample.com%2Fuser%2Fapi%2FtoolsImport%2Ffetch%3Fid%3Dabc%26token%3Ddef&auto_import=true";
        let payload = parse_external_import_url(raw).expect("payload");
        assert_eq!(payload.provider_id, "codex");
        assert_eq!(payload.page, "codex");
        assert_eq!(payload.token, "");
    ...
    fn parse_import_link_with_api_base_url() {
        let raw = "cle-console://provider-import?platform=codex&import_url=https%3A%2F%2Fchongflow.cn%2Fapi%2Fcle-console%2Fimport%2Fabc&api_base_url=https%3A%2F%2Fchongflow.cn%2Fv1&auto_import=true";
        let payload = parse_external_import_url(raw).expect("payload");
        assert_eq!(payload.provider_id, "codex");
        assert_eq!(
    ...
    fn parse_activate_flag() {
        let raw = "cle-console://provider-import?platform=codex&import_url=https%3A%2F%2Fexample.com%2Fbundle&auto_import=true&activate=true";
        let payload = parse_external_import_url(raw).expect("payload");
        assert_eq!(payload.provider_id, "codex");
        assert!(payload.auto_import);
    ...
    fn parse_min_app_version() {
        let raw = "cle-console://import?provider=codex&token=abc123&min_app_version=v0.22.21";
        let payload = parse_external_import_url(raw).expect("payload");
        assert_eq!(payload.provider_id, "codex");
