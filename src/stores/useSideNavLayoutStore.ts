...
export const useSideNavLayoutStore = create<SideNavLayoutState>()(
  persist(
    (set) => ({
      mode: getOldStorage<SideNavLayoutMode>('agtools.side_nav.layout.v1', (v) => (v === 'original' ? 'original' : 'classic'), 'classic'),
    ...
    {
      // The industrial redesign uses the labeled tool rail as its default shell.
      // A new layout-only key lets existing installations receive that default
      // without touching any account, provider, or application preferences.
      name: 'agtools.side_nav.store.v3',
