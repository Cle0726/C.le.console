...
function normalizeProviderExternalUrl(value?: string | null): string {
  const trimmed = value?.trim();
  if (!trimmed) return "";
  try {
...
const DEFAULT_PROVIDER_PREVIEW_PATHS: ProviderPreviewPaths = {
  providerStorePath: "~/.antigravity_cle/codex_model_providers.json",
    ...
    void (async () => {
      try {
        ...
        const [providerStorePath, codexConfigPath, codexAuthPath] =
          await Promise.all([
            join(home, ".antigravity_cle", "codex_model_providers.json"),
                  ...
                  {usageMode === "sub2api" ? (
                    ...
                    <div className="codex-api-key-usage-panel">
                      <div
                        ...
                        title={`${t("codex.cleApi.balance", "额度")}：${
                          usageSummary?.quotaUnlimited
                  ...
                  <div className="card-footer">
                    <div className="card-actions">
                      ...
                      <button
                        className="card-action-btn"
                        onClick={() => {
                          const targetUrl = normalizeProviderExternalUrl(
                            provider.website ||
                            provider.apiKeyUrl ||
                            provider.baseUrl,
                          );
