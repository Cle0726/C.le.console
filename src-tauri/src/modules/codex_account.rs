...
const CLE_API_LOGIN_PLAN_TYPE: &str = "Cle Api";
const CLE_API_DEFAULT_ACCOUNT_NAME: &str = "Codex API";
...
const CODEX_MANAGED_MODEL_CATALOG_FILE: &str = "cle-provider-model-catalog.json";
const CODEX_AUTO_REVIEW_MODEL_ID: &str = "codex-auto-review";
const CODEX_DEFAULT_OPENAI_BASE_URL: &str = "https://api.openai.com/v1";
const CODEX_CLE_API_BASE_URL: &str = "https://chongcodex.cn/v1";
const CODEX_CLE_API_PROVIDER_ID: &str = "cle_api";
...
const CODEX_AUTH_PROJECTION_FILE_NAME: &str = ".cle_codex_auth.json";
const CODEX_AUTH_PROJECTION_WRITER: &str = "cle";
...
fn is_cle_api_base_url(raw: Option<&str>) -> bool {
    let Some(actual) = normalize_api_base_url_for_match(raw) else {
        return false;
    };
    let Some(expected) = normalize_api_base_url_for_match(Some(CODEX_CLE_API_BASE_URL)) else {
        return false;
    };
    actual == expected
}
...
fn apply_api_key_fields(
    ...
    let is_cle_api = provider_config
        .provider_id
        .as_deref()
        .map(|value| value.eq_ignore_ascii_case(CODEX_CLE_API_PROVIDER_ID))
        .unwrap_or(false)
        || is_cle_api_base_url(provider_config.base_url.as_deref());
    let plan_type = if is_cle_api {
        CLE_API_LOGIN_PLAN_TYPE
    } else {
    ...
    if is_cle_api && normalize_optional_ref(account.account_name.as_deref()).is_none() {
        account.account_name = Some(CLE_API_DEFAULT_ACCOUNT_NAME.to_string());
    }
...
fn collect_managed_api_key_provider_ids() -> HashSet<String> {
    let mut ids = HashSet::from([
        ...
        CODEX_CLE_API_PROVIDER_ID.to_string(),
...
/// 旧版数据目录（~/Library/Application Support/com.antigravity.cle-console/）
fn get_old_codex_data_dir() -> PathBuf {
    dirs::data_local_dir()
        .unwrap_or_else(|| dirs::home_dir().expect("无法获取用户目录"))
        .join("com.antigravity.cle-console")
}
...
/// 获取账号详情存储目录（统一使用 ~/.antigravity_cle/codex_accounts/）
...
fn load_account_with_summary(
    ...
    if let Ok(mut account) = serde_json::from_str::<CodexAccount>(&content) {
        ...
        let migrated_bound_oauth = raw_value
            ...
            .unwrap_or(false);
        if migrated_bound_oauth || migrated_index_summary {
...
pub fn sync_managed_projection_from_auth_dir(
    ...
    let projection = read_managed_projection_from_dir(base_dir)
        .ok_or_else(|| "目标目录不是 Cle 受管 Codex 投影，已拒绝反向同步".to_string())?;
...
fn build_auth_file_value(account: &CodexAccount) -> Result<serde_json::Value, String> {
    ...
    serde_json::to_value(CodexAuthFile {
        ...
        tokens: Some(CodexAuthTokens {
            ...
            // Cle can switch short-lived opaque `at-...` credentials without
...
fn get_codex_batch_import_sessions_dir() -> PathBuf {
    let data_dir = account::get_data_dir()
        ...
        .unwrap_or_else(|_| PathBuf::from(".antigravity_cle"));
...
{"id_token":"id-2","access_token":"access-2"}"#;
    ...
    fn assert_builtin_openai_provider_selection(content: &str) {
        #[cfg(target_os = "windows")]
        assert!(content
            .lines()
            .any(|line| line.trim() == r#"model_provider = "openai""#));
        #[cfg(not(target_os = "windows"))]
    ...
    impl TestEnvGuard {
        fn new(prefix: &str) -> Self {
            ...
            let previous_data_dir = std::env::var("CLE_CONSOLE_DATA_DIR").ok();
            std::env::set_var("HOME", &home_dir);
            std::env::set_var("CODEX_HOME", &codex_home);
            std::env::set_var("CLE_CONSOLE_DATA_DIR", &home_dir);
    ...
    impl Drop for TestEnvGuard {
        fn drop(&mut self) {
            ...
            match self.previous_data_dir.as_ref() {
                Some(value) => std::env::set_var("CLE_CONSOLE_DATA_DIR", value),
                None => std::env::remove_var("CLE_CONSOLE_DATA_DIR"),
            }
        ...
        let content = r#"[
  {
    ...
    fn config_toml_uses_openai_base_url_for_builtin_openai() {
        ...
        assert_builtin_openai_provider_selection(&content);
...
model_catalog_json = "cle-provider-model-catalog.json"
...
"#,
        ...
        assert!(!content.contains("[model_providers.cle_api]"));
...
"#,
        ...
        assert!(content.contains("[model_providers.openai_api_key]"));
        assert!(content.contains("[model_providers.relay]"));
        assert!(content.contains("model_catalog_json = \"cle-provider-model-catalog.json\""));
