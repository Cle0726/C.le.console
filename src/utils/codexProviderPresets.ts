import { isNeutralProviderPreset } from './providerPresetVisibility';
...
export const CLE_API_PROVIDER_ID = "cle_api";
export const CLE_API_PROVIDER_NAME = "Cle Api";
export const CLE_API_BASE_URL = "https://chongcodex.cn/v1";

const CLE_API_HIDDEN_BASE_URLS = [CLE_API_BASE_URL] as const;

const CODEX_API_PROVIDER_PRESET_CATALOG: readonly CodexApiProviderPreset[] = [
  {
    id: "openai_official",
...
export const CODEX_API_PROVIDER_PRESETS: readonly CodexApiProviderPreset[] =
  CODEX_API_PROVIDER_PRESET_CATALOG.filter(isNeutralProviderPreset);
...
export function isCleApiProviderBaseUrl(rawBaseUrl: string): boolean {
  const normalized = normalizeCodexProviderBaseUrl(rawBaseUrl);
  if (!normalized) return false;
  return CLE_API_HIDDEN_BASE_URLS.some(
    (baseUrl) => normalizeCodexProviderBaseUrl(baseUrl) === normalized,
  );
}
