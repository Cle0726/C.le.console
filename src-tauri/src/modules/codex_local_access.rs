...
const CODEX_LOCAL_ACCESS_SIDECAR_BIN_NAME: &str = "cle-cliproxy";
...
const CODEX_LOCAL_ACCESS_API_PORT_ENV: &str = "CLE_CONSOLE_API_PORT";
...
const CODEX_PROVIDER_MODEL_CATALOG_FILE: &str = "cle-provider-model-catalog.json";
const CODEX_PROVIDER_MODEL_BACKUP_FILE: &str = ".cle-provider-model-backup.json";
...
fn sidecar_binary_candidates() -> Result<Vec<PathBuf>, String> {
    ...
    let dev_sidecar_dir = manifest_dir.join("../sidecars/cle-cliproxy/bin");
...
pub fn sync_sidecar_auth_file_for_account(account: &CodexAccount) -> Result<(), String> {
    ...
    logger::log_codex_api_info(&format!(
        "[CodexLocalAccess][sidecar] 已写穿 Cle Token Authority 凭证: account_id={}",
        account.id
    ));
    Ok(())
}
...
fn is_model_provider_image_test(model_id: &str) -> bool {
    model_id.trim().eq_ignore_ascii_case(CODEX_IMAGE_MODEL_ID)
}

fn count_valid_model_provider_test_images(body: &Value) -> usize {
    body.get("data")
        .and_then(Value::as_array)
        .into_iter()
        .flatten()
...
fn build_model_provider_test_request_payload(
    request: &CodexModelProviderGatewayChatTestRequest,
    client_model_id: &str,
    client_request_id: &str,
) -> (&'static str, Value) {
    if is_model_provider_image_test(&request.model_id) {
        return (
            IMAGES_GENERATIONS_PATH,
            json!({
    ...
    if model_provider_test_uses_provider_gateway(request) {
        (
            RESPONSES_PATH,
            json!({
                ...
                "metadata": {
                    ...
                    "agtools_test_model": request.model_id.trim(),
                    "agtools_client_model": client_model_id,
...
pub async fn run_model_provider_gateway_chat_test(
    ...
    let client_model_id = if uses_provider_gateway || is_model_provider_image_test(model_id) {
        model_id.to_string()
    } else {
    ...
    let (request_path, body) =
        build_model_provider_test_request_payload(&request, &client_model_id, &client_request_id);
    let url = format!("http://{}:{}{}", bind_host, collection.port, request_path);
    ...
    let result = async {
        ...
        let reply = if is_model_provider_image_test(model_id) {
            let parsed = serde_json::from_str::<Value>(&text)
                .map_err(|error| format!("解析本地网关生图响应失败: {}", error))?;
            let image_count = count_valid_model_provider_test_images(&parsed);
            (image_count > 0).then(|| format!("生图成功，共返回 {} 张图片", image_count))
        } else if uses_provider_gateway {
            let parsed = serde_json::from_str::<Value>(&text)
        ...
        .ok_or_else(|| {
            if is_model_provider_image_test(model_id) {
                "本地网关生图未返回图片数据".to_string()
            } else {
                "本地网关对话未返回可读回复".to_string()
            }
        })?;
...
"#;
    ...
    fn provider_gateway_writes_model_catalog_json() {
        let profile_dir = std::env::temp_dir().join(format!(
            "cle-provider-model-catalog-test-{}",
    ...
    fn provider_gateway_cleanup_removes_managed_model_override() {
        let profile_dir = std::env::temp_dir().join(format!(
            "cle-provider-model-cleanup-test-{}",
            chrono::Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
        fs::create_dir_all(&profile_dir).expect("create temp profile");
    ...
    fn provider_gateway_cleanup_restores_previous_official_model() {
        let profile_dir = std::env::temp_dir().join(format!(
            "cle-provider-model-restore-test-{}",
            chrono::Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
        fs::create_dir_all(&profile_dir).expect("create temp profile");
    ...
    fn provider_gateway_cleanup_restores_previous_model_without_catalog_file() {
        let profile_dir = std::env::temp_dir().join(format!(
            "cle-provider-model-restore-no-catalog-test-{}",
            chrono::Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
        fs::create_dir_all(&profile_dir).expect("create temp profile");
    ...
    #[test]
    fn provider_gateway_cleanup_keeps_non_cle_model_catalog() {
        let profile_dir = std::env::temp_dir().join(format!(
            "cle-provider-model-keep-external-catalog-test-{}",
            chrono::Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
        fs::create_dir_all(&profile_dir).expect("create temp profile");
...
"#;
    ...
    #[test]
    fn model_provider_image_test_uses_images_generations_endpoint() {
        ...
        assert_eq!(body.get("prompt"), Some(&json!("hello")));
    ...
    #[test]
    fn model_provider_image_test_only_accepts_supported_image_model() {
        assert!(super::is_model_provider_image_test("GPT-IMAGE-2"));
        assert!(!super::is_model_provider_image_test("gpt-image-1"));
        assert!(!super::is_model_provider_image_test("dall-e-3"));
        assert!(!super::is_model_provider_image_test("custom-image-gen"));
    }
    ...
    fn model_provider_image_test_requires_decodable_image_data() {
        ...
        assert_eq!(
            count_valid_model_provider_test_images(&json!({"data": [{"b64_json": "aW1hZ2U="}]})),
            1
        );
    }

    #[test]
    fn model_provider_responses_test_keeps_direct_chat_adapter_endpoint() {
    ...
    #[test]
    fn model_provider_image_test_collection_allows_requested_image_model() {
        let mut request = model_provider_chat_test_request("responses");
        request.model_id = "gpt-image-2".to_string();
        let account = CodexAccount::new_api_key(
            "api-test-image".to_string(),
            "api-key@example.com".to_string(),
            "sk-test".to_string(),
            CodexApiProviderMode::Custom,
            Some("https://relay.example/v1".to_string()),
        ...
        assert!(collection.model_aliases.iter().any(|alias| {
            alias.source_model.eq_ignore_ascii_case("gpt-image-2")
                && alias.alias.eq_ignore_ascii_case("gpt-image-2")
        }));
    }
