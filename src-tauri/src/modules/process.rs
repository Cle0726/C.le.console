...
fn command_trace_enabled() -> bool {
    if let Ok(value) = std::env::var("CLE_COMMAND_TRACE") {
...
fn should_detach_child() -> bool {
    if let Ok(value) = std::env::var("CLE_CHILD_LOGS") {
        let lowered = value.trim().to_lowercase();
        if matches!(lowered.as_str(), "1" | "true" | "yes" | "on") {
            return false;
        }
    }
    if let Ok(value) = std::env::var("CLE_CHILD_DETACH") {
        let lowered = value.trim().to_lowercase();
...
fn sanitize_macos_gui_launch_env(cmd: &mut Command) {
    // Avoid inheriting Cle bundle identity into child GUI apps.
...
fn is_legacy_platform_adapter_executable(executable: &str) -> bool {
    ...
    file_name.starts_with("cle-") && file_name.ends_with("-adapter")
}
...
pub fn close_codex_instances(codex_homes: &[String], timeout_secs: u64) -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if includes_default
            && std::env::var("CLE_CODEX_CLOSE_RESOURCE_CLEANUP")
                .ok()
                .as_deref()
                == Some("1")
        {
...
mod legacy_platform_adapter_cleanup_tests {
    ...
    fn matches_orphaned_legacy_platform_adapter() {
        let line = " 1359     1 /Users/jieli/.antigravity_cle/platform-packages/codex/current/adapter/macos/cle-codex-adapter";
        assert_eq!(
    ...
    fn ignores_non_orphaned_or_current_processes() {
        let line = " 1359 1805 /Users/jieli/.antigravity_cle/platform-packages/codex/current/adapter/macos/cle-codex-adapter";
        assert_eq!(
            orphaned_legacy_platform_adapter_pid_from_ps_line(line, 99999),
            None
        );

        let current_line = " 1359 1 /Users/jieli/.antigravity_cle/platform-packages/codex/current/adapter/macos/cle-codex-adapter";
        assert_eq!(
    ...
    fn ignores_current_sidecar_and_official_apps() {
        let sidecar =
            " 64680 1805 /Applications/C.le.控制台.app/Contents/MacOS/cle-cliproxy --parent-pid 1805";
        assert_eq!(
...
mod tests {
    ...
    fn trae_windows_scan_candidates_match_exact_platform_dirs() {
        let trae = Path::new(r"D:\Users\TestUser\AppData\Local\Programs\Trae\Trae.exe");
        let trae_cn = Path::new(r"D:\Users\TestUser\AppData\Local\Programs\Trae CN\Trae CN.exe");
        let solo_cn =
            Path::new(r"D:\Users\TestUser\AppData\Local\Programs\TRAE SOLO CN\TRAE SOLO CN.exe");
