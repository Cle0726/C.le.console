...
export type CodexExportFormat = 'cle_console' | 'sub2api' | 'cpa';
...
function buildAgentIdentityCredentials(account: CodexAccount): JsonRecord {
  ...
  if (identity?.task_id?.trim()) credentials.task_id = identity.task_id.trim();
  if (identity?.email?.trim() || account.email?.trim()) {
    credentials.email = identity?.email?.trim() || account.email.trim();
  }
  if (identity?.plan_type?.trim() || account.plan_type?.trim()) {
    credentials.plan_type = identity?.plan_type?.trim() || account.plan_type?.trim();
  }
  return credentials;
}
...
function toPortableAgentIdentityStorage(
  ...
  if (options.includeSensitiveNotes) appendSensitiveNoteFields(payload, account);
  return payload;
}
...
function toCleConsolePortableStorage(
  account: CodexAccount,
  options: CodexExportBuildOptions = {},
...
export function parseCleConsoleCodexExport(rawJson: string): CodexAccount[] {
...
export function hasCodexExportSensitiveNotes(rawJson: string): boolean {
  try {
    return parseCleConsoleCodexExport(rawJson).some(hasSensitiveNoteFields);
  } catch {
    return false;
  }
}
...
export function hasCodexExportAgentIdentity(rawJson: string): boolean {
  try {
    return parseCleConsoleCodexExport(rawJson).some(hasAgentIdentity);
  } catch {
    return false;
  }
}
...
export function transformCodexExportJson(
  ...
  const accounts = parseCleConsoleCodexExport(rawJson);

  if (format === 'cle_console') {
    return JSON.stringify(
      accounts.map((account) => toCleConsolePortableStorage(account, options)),
      null,
      2,
    );
  }
