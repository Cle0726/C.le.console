...
const CLE_API_BASE_URL = "https://chongcodex.cn/v1";
...
export function isCodexCleApiBaseUrl(rawValue?: string | null): boolean {
  return (
    normalizeCodexApiBaseUrlForMatch(rawValue) ===
    normalizeCodexApiBaseUrlForMatch(CLE_API_BASE_URL)
  );
}
...
export function isCodexNewApiAccount(account: CodexAccount): boolean {
  ...
  return (
    ...
    (providerId === "cle_api" ||
      providerId === "new_api" ||
      isCodexCleApiBaseUrl(account.api_base_url) ||
      planType === "CLE API" ||
...
function getCodexPlanBadgeLabel(account: CodexAccount): string {
  if (isCodexNewApiAccount(account)) {
    const planType = account.plan_type?.trim();
    return !planType || planType.toLowerCase() === "cle api"
      ? "C.le. API"
      : planType;
  }
