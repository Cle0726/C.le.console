...
const ACCOUNT_BILINGUAL_TAB_LABELS: Record<PlatformOverviewTab, { zh: string; en: string }> = {
  overview: { zh: '账户总览', en: 'Accounts' },
  providers: { zh: '模型供应商', en: 'Providers' },
  wakeup: { zh: '唤醒任务', en: 'Wake' },
  instances: { zh: '应用多开', en: 'Instances' },
  sessions: { zh: '会话管理', en: 'Sessions' },
};

function BilingualShellLabel({ zh, en }: { zh: string; en: string }) {
  return (
    <span className="account-bilingual-label">
      <span className="account-bilingual-label-zh">{zh}</span>
      <span className="account-bilingual-label-en">{en}</span>
    </span>
  );
}
...
export function PlatformOverviewTabsHeader({
  ...
  const remoteHiddenPlatformSet = useMemo(() => new Set<PlatformId>(), []);
    ...
    <>
      <div className="page-top-strip">
        <div className="page-top-strip-left">
          <span className="page-top-strip-label account-workspace-kicker">
            <span>{t('overview.accountWorkspace', '账户工作区')}</span>
        ...
        <div className="page-tabs filter-tabs">
          {tabSpecs.map((tab) => (
            ...
            >
              ...
              <BilingualShellLabel
                zh={tab.label}
                en={ACCOUNT_BILINGUAL_TAB_LABELS[tab.key].en}
              />
            </button>
          ))}
        </div>
