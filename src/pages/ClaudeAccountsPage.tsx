...
function getNextClaudeDesktopGatewaySafeModel(usedModels: Iterable<string>): string {
  const used = new Set(
    Array.from(usedModels)
      .map((model: any) => model.trim().toLowerCase())
      .filter(Boolean),
  );
  const defaultModel = DEFAULT_CLAUDE_DESKTOP_GATEWAY_MODELS.find(
    (model: any) => !used.has(model.toLowerCase()),
  );
...
function buildClaudeDesktopGatewayMappings(
  ...
  const normalizedDesktopModels = desktopModels.map((model: any) => model.trim()).filter(Boolean);
  const normalizedUpstreamModels = upstreamModels.map((model: any) => model.trim()).filter(Boolean);
...
function buildClaudeDesktopGatewayDesktopModelOptions(
  ...
  const options = DEFAULT_CLAUDE_DESKTOP_GATEWAY_MODELS
    ...
    .map((model: any) => ({ value: model, label: model }));
  return [
    ...options,
...
export function ClaudeAccountsPage({ subPlatform = 'desktop' }: ClaudeAccountsPageProps) {
  ...
  const remoteHiddenPlatformIds: string[] = [];
  ...
  const openEditDesktopGatewayModal = (account: ClaudeAccount) => {
    ...
    const savedModels = (account.desktop_gateway_models || []).map((model: any) => model.trim()).filter(Boolean);
    ...
    const upstreamModels = (
      account.desktop_gateway_upstream_models?.length
        ...
        : savedModelsAreClaude
          ? []
          : savedModels
    ).map((model: any) => model.trim()).filter(Boolean);
  ...
  const applyApiKeyFunPrefill = useCallback(
    (request: any) => {
      ...
      if (request.target === 'claude_cli') {
        ...
        setAddTab('apikey');
        setApiProviderPresetId("apikey_fun");
        setApiBaseUrlInput(request.baseUrl?.trim() || "https://api.apikey.fun");
        setApiProviderTemplateValues({});
  ...
  const handleFetchDesktopGatewayModels = async () => {
    ...
    try {
      ...
      const models = result.models.map((model: any) => model.id.trim()).filter(Boolean);
      if (models.length === 0) {
  ...
  const handleImportApiKey = async () => {
    ...
    const directDesktopGatewayModelSet = new Set(
      directDesktopGatewayModels.map((model: any) => model.toLowerCase()),
    );
    ...
    if (addTab === 'desktopGateway') {
      if (desktopGatewayConnectionMode === 'local_mapping') {
        ...
        if (desktopGatewayModels.some((model: any) => !isClaudeDesktopGatewayRouteModel(model))) {
