...
pub fn managed_instances_root_dir() -> Result<PathBuf, String> {
    #[cfg(target_os = "macos")]
    {
        ...
        return Ok(home.join(".antigravity_cle/instances/antigravity"));
    }

    #[cfg(target_os = "windows")]
    {
        let roaming_dir = roaming_app_data_dir()?;
        return Ok(roaming_dir.join(".antigravity_cle\\instances\\antigravity"));
    }
...
pub fn legacy_managed_instances_root_dir() -> Result<PathBuf, String> {
    ...
    {
        let home = dirs::home_dir().ok_or("无法获取用户主目录")?;
        return Ok(home.join(".antigravity_cle/instances/antigravity-legacy"));
    }

    #[cfg(target_os = "windows")]
    {
        let roaming_dir = roaming_app_data_dir()?;
        return Ok(roaming_dir.join(".antigravity_cle\\instances\\antigravity-legacy"));
    }
