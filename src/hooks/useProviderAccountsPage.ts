...
const CLE_API_PROVIDER_ID = 'cle_api';
const CLE_API_PROVIDER_NAME = 'Cle Api';
const CLE_CONSOLE_IMPORT_PATH_MARKERS = [
  '/api/cle-console/import/',
...
const isCleConsoleImportUrl = (importUrl?: string | null): boolean => {
  const trimmed = (importUrl || '').trim();
  if (!trimmed) return false;
  try {
    const parsed = new URL(trimmed);
    const pathname = parsed.pathname.toLowerCase();
    return CLE_CONSOLE_IMPORT_PATH_MARKERS.some((marker) => pathname.includes(marker));
  } catch {
    return false;
  }
};

const isCleApiImportItem = (item: Record<string, unknown>): boolean => {
  const providerId = readRecordString(item, ['api_provider_id', 'apiProviderId']);
  if (providerId?.toLowerCase() === CLE_API_PROVIDER_ID) return true;
  ...
  const expected = CLE_API_PROVIDER_NAME.toLowerCase();
  return candidates.some(
    (value) => value?.trim().toLowerCase().includes(expected),
  );
};

const withCleApiBaseUrl = (
  item: unknown,
  apiBaseUrl: string | null,
  isCleConsoleImport: boolean,
): unknown => {
  ...
  if (!isCleConsoleImport && !isCleApiImportItem(payload)) {
    return item;
  }
  ...
  return {
    ...
    api_provider_id:
      readRecordString(payload, ['api_provider_id', 'apiProviderId']) ?? CLE_API_PROVIDER_ID,
    api_provider_name:
      readRecordString(payload, ['api_provider_name', 'apiProviderName']) ??
      CLE_API_PROVIDER_NAME,
    plan_type:
      readRecordString(payload, ['plan_type', 'planType']) ?? CLE_API_PROVIDER_NAME,
  };
};

const applyCleApiBaseUrlToExternalImportItems = (
  items: unknown[],
  ...
  const isCleConsoleImport =
    Boolean(request.apiBaseUrl?.trim()) || isCleConsoleImportUrl(request.importUrl);
  return items.map((item) => withCleApiBaseUrl(item, apiBaseUrl, isCleConsoleImport));
};
    ...
    (request: ExternalProviderImportPayload) => {
      ...
      void (async () => {
        try {
          ...
          const items =
            platformId === 'codex'
              ? applyCleApiBaseUrlToExternalImportItems(resolvedItems, request)
              : resolvedItems;
