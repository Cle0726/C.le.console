...
const MODEL_CATALOG_FILE: &str = "cle-provider-model-catalog.json";
    ...
    match method {
        "Runtime.consoleAPICalled" => {
            ...
            let text = params
                ...
                .unwrap_or_default();
            if text.contains("Cle") || text.contains("features") || text.contains("model") {
                crate::modules::logger::log_info(&format!(
...
(() => {{
  ...
  if (window.__cleCodexModelInjectorVersion === "2") return;
  window.__cleCodexModelInjectorVersion = "2";
  try {{ console.info("[Cle Codex] model injector active", modelNames); }} catch {{}}
  ...
  function patchModelPayload(payload) {{
    ...
    try {{
      ...
      try {{ console.warn("[Cle Codex] model payload patch failed", error?.message || String(error)); }} catch {{}}
    }}
    return payload;
  }}
  ...
  function patchStatsig() {{
    ...
    for (const client of clients) {{
      if (typeof client.getDynamicConfig !== "function" || client.__cleModelPatched) continue;
  ...
  function patchClient(client) {{
    if (!client || typeof client.sendRequest !== "function" || client.__cleModelRequestPatched) return false;
    ...
    client.sendRequest = async function patchedSendRequest(method, params, options) {{
      ...
      if (shouldTrace) {{
        try {{ console.info("[Cle Codex] app-server request", actualMethod, stringifyForLog(params)); }} catch {{}}
      }}
      try {{
        ...
        if (shouldTrace) {{
          try {{ console.info("[Cle Codex] app-server response", actualMethod, stringifyForLog(result)); }} catch {{}}
        }}
        return result;
      }} catch (error) {{
        if (shouldTrace || String(error?.message || error || "").includes("features")) {{
          try {{ console.warn("[Cle Codex] app-server error", actualMethod, stringifyForLog(params), error?.message || String(error), error?.stack || ""); }} catch {{}}
        }}
        throw error;
      }}
    }};
    client.__cleModelRequestPatched = true;
    return true;
  }}
