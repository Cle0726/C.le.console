...
const SENTRY_CLIENT: &str = "cle-console/1.0";
...
fn build_sentry_event(
    ...
    let mut event = json!({
        ...
        "platform": "rust",
        "level": level,
        "release": format!("cle-console@{}", env!("CARGO_PKG_VERSION")),
...
fn current_dsn() -> Option<String> {
    let runtime = std::env::var("CLE_SENTRY_DSN")
        .ok()
        .or_else(|| std::env::var("SENTRY_DSN").ok())
        .or_else(|| option_env!("CLE_SENTRY_DSN").map(ToString::to_string))
...
fn should_send(level: &str, message: &str, source: Option<&str>, phase: Option<&str>) -> bool {
    let env_disabled = std::env::var("CLE_DISABLE_ERROR_REPORTING")
        .ok()
        .map(|value| {
