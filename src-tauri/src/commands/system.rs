...
/// Detect the user's effective proxy configuration and return only controller-observed
/// routes/rules. Unknown values deliberately remain null/empty rather than using demo data.
#[tauri::command]
pub async fn get_network_egress_snapshot(
) -> Result<crate::modules::network_egress::NetworkEgressSnapshot, String> {
    Ok(crate::modules::network_egress::get_network_egress_snapshot().await)
}
...
fn read_powershell_json_for_antigravity_exe(
    ...
    let output = command
        ...
        .env("CLE_ANTIGRAVITY_EXE_PATH", exe_path.as_os_str())
        .output()
        .ok()?;
    if !output.status.success() {
...
$exe = [Environment]::GetEnvironmentVariable('CLE_ANTIGRAVITY_EXE_PATH', 'Process')
...
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
$OutputEncoding = [System.Text.Encoding]::UTF8
$p = [Environment]::GetEnvironmentVariable('CLE_ANTIGRAVITY_EXE_PATH', 'Process')
...
fn build_auto_backup_zip_bytes(file_name: &str, content: &str) -> Result<Vec<u8>, String> {
    ...
    let manifest = serde_json::json!({
        "schema": "cle-console.auto-backup-archive",
        "version": 1,
...
#[tauri::command]
pub fn show_status_window(app: tauri::AppHandle) -> Result<(), String> {
    modules::status_window::show_status_window(&app)
}

#[tauri::command]
pub fn hide_status_window(app: tauri::AppHandle) -> Result<(), String> {
    modules::status_window::hide_status_window(&app)
}
...
pub async fn start_chat2api() -> Result<(), String> {
    ...
    #[cfg(not(target_os = "windows"))]
    Err("Chat2API 快捷启动目前仅支持 Windows".to_string())
}
