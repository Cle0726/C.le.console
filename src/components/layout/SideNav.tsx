...
const APP_DISPLAY_NAME =
  import.meta.env.VITE_CLE_CONSOLE_PROFILE === 'dev' ? 'C.le.控制台 Dev' : 'C.le.控制台';

const CLASSIC_NAV_MIN_SCALE = 0.5;
const CLASSIC_NAV_SCALE_EPSILON = 0.004;
const CLASSIC_NAV_SCROLL_EPSILON = 4;
const SPATIAL_NAV_ACTIONS_ID = 'spatial-navigation-actions';
const SPATIAL_NAV_MORE_ID = 'spatial-navigation-more';
...
export function SideNav({
  ...
  const spatialNavRef = useRef<HTMLElement>(null);
  const spatialAvatarRef = useRef<HTMLButtonElement>(null);
  const spatialFirstActionRef = useRef<HTMLButtonElement>(null);
  ...
  const isPlatformAvailable = useCallback(
    (platformId: PlatformId) => isMenuVisiblePlatform(platformId),
    [],
  );
  ...
  const closeSpatialNavigation = useCallback((restoreAvatarFocus = false) => {
    setShowMore(false);
    setSpatialNavExpanded(false);

    if (restoreAvatarFocus && typeof window !== 'undefined') {
      window.requestAnimationFrame(() => spatialAvatarRef.current?.focus());
    }
  }, []);

  const handleSpatialAvatarClick = useCallback((event: ReactMouseEvent<HTMLButtonElement>) => {
    const opening = !spatialNavExpanded;
    setShowMore(false);
    setSpatialNavExpanded(opening);

    // Keyboard activation should continue directly into the revealed actions.
    if (opening && event.detail === 0 && typeof window !== 'undefined') {
      window.requestAnimationFrame(() => spatialFirstActionRef.current?.focus());
    }
  }, [spatialNavExpanded]);
  ...
  useEffect(() => {
    if (!spatialNavExpanded) return;

    const handleSpatialClickOutside = (event: MouseEvent) => {
      const target = event.target as Node;
      if (spatialNavRef.current?.contains(target)) return;
      closeSpatialNavigation();
    };

    document.addEventListener('mousedown', handleSpatialClickOutside);
    return () => document.removeEventListener('mousedown', handleSpatialClickOutside);
  }, [closeSpatialNavigation, spatialNavExpanded]);

  useEffect(() => {
    if (!spatialNavExpanded) return;

    const handleSpatialEscape = (event: KeyboardEvent) => {
    ...
    document.addEventListener('keydown', handleSpatialEscape);
    return () => document.removeEventListener('keydown', handleSpatialEscape);
  }, [closeSpatialNavigation, showMore, spatialNavExpanded]);
  ...
  const spatialNavEntries = sidebarMenuEntries.slice(0, 8);
  const spatialNavSplit = Math.ceil(spatialNavEntries.length / 2);
  const spatialLeftEntries = spatialNavEntries.slice(0, spatialNavSplit);
  const spatialRightEntries = spatialNavEntries.slice(spatialNavSplit);
  const renderSpatialEntry = (entry: SideNavEntry) => {
    const active = currentEntryId === entry.id && !shouldLockActiveOnMore;
    return (
      <button
        key={entry.id}
        className={`spatial-nav-item${active ? ' active' : ''}`}
        onClick={() => {
          closeSpatialNavigation(true);
          navigateToEntry(entry);
        }}
      ...
      >
        {renderEntryIcon(entry, 18)}
        <span className="spatial-nav-tooltip">{entry.label}</span>
      </button>
    );
  };
  ...
  return (
    <>
      <nav
        ref={spatialNavRef}
        className={`spatial-top-nav${spatialNavExpanded ? ' expanded' : ''}`}
        aria-label="主导航"
      >
        <div
          id={SPATIAL_NAV_ACTIONS_ID}
          className="spatial-nav-actions"
          hidden={!spatialNavExpanded}
          aria-hidden={!spatialNavExpanded}
        >
          {spatialNavExpanded && (
            <>
              <div className="spatial-nav-wing spatial-nav-wing-left">
                ...
                >
                  <GaugeCircle size={18} />
                  <span className="spatial-nav-tooltip">{t('nav.dashboard')}</span>
                </button>
                {spatialLeftEntries.map(renderSpatialEntry)}
                <button
                ...
                >
                  <FileText size={18} />
                  <span className="spatial-nav-tooltip">{t('nav.logs', '日志')}</span>
                </button>
              </div>
              ...
              <div className="spatial-nav-wing spatial-nav-wing-right">
                ...
                <button
                  ...
                  aria-current={page === '2fa' ? 'page' : undefined}
                >
                  <ShieldCheck size={18} />
                  <span className="spatial-nav-tooltip">2FA</span>
                </button>
                <button
                  className={`spatial-nav-item${page === 'settings' ? ' active' : ''}`}
                  onClick={() => {
                    closeSpatialNavigation(true);
                    setPage('settings');
                  }}
                ...
                >
                  <Settings size={18} />
                  <span className="spatial-nav-tooltip">{t('nav.settings')}</span>
                </button>
                <button
                  ref={moreButtonRef}
                  className={`spatial-nav-item${showMore || isMoreActive ? ' active' : ''}`}
                  onClick={() => setShowMore((value) => !value)}
                  title={t('nav.morePlatforms', '更多平台')}
                  aria-expanded={showMore}
                  aria-controls={SPATIAL_NAV_MORE_ID}
                  aria-haspopup="true"
                  aria-current={isMoreActive ? 'page' : undefined}
                >
                  <LayoutGrid size={18} />
                  <span className="spatial-nav-tooltip">{t('nav.morePlatforms', '更多平台')}</span>
                </button>
              </div>
            </>
          )}
        ...
        <div className="spatial-nav-core">
          <span className="spatial-floating-brand">C.le.</span>
          <button
            ref={spatialAvatarRef}
            className="spatial-nav-avatar"
            onClick={handleSpatialAvatarClick}
            aria-label={spatialNavExpanded ? '收起导航' : '展开导航'}
            aria-expanded={spatialNavExpanded}
            aria-controls={SPATIAL_NAV_ACTIONS_ID}
          >
        ...
        {showMore && spatialNavExpanded && (
          <div
            ref={morePopoverRef}
            id={SPATIAL_NAV_MORE_ID}
            className="spatial-nav-more-panel"
  ...
  /* Legacy side navigation is retained below for reference during migration. */
        ...
        <div className="side-nav-brand-main">
          ...
          >
            <img
              ...
              style={{ width: isClassicLayout ? classicBrandLogoIconSize + 12 : 42, height: isClassicLayout ? classicBrandLogoIconSize + 12 : 42 }}
            />
