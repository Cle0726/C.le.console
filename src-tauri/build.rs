...
fn should_skip_sidecar_build(output: &Path) -> bool {
    std::env::var("CLE_SKIP_CLIPROXY_BUILD").ok().as_deref() == Some("1") && output.exists()
}
...
fn build_go_sidecar(
    ...
    let output = output_dir.join(format!("cle-cliproxy-{rust_target}{extension}"));
    ...
    let status = Command::new("go")
        ...
        .arg(".")
        .status()
        .expect("failed to start go build for cle-cliproxy");

    if !status.success() {
        panic!("go build for cle-cliproxy failed with status: {status}");
    }
...
fn build_macos_universal_sidecar(sidecar_dir: &Path, output_dir: &Path) {
    let output = output_dir.join("cle-cliproxy-universal-apple-darwin");
    if should_skip_sidecar_build(&output) {
        return;
    }
    ...
    let status = Command::new("lipo")
        ...
        .arg(&output)
        .status()
        .expect("failed to start lipo for cle-cliproxy universal sidecar");

    if !status.success() {
        panic!("lipo for cle-cliproxy universal sidecar failed with status: {status}");
    }
}

fn build_cle_cliproxy_sidecar() {
    let manifest_dir =
    ...
    println!("cargo:rustc-env=CLE_RUST_TARGET={target}");
    let sidecar_dir = manifest_dir.join("../sidecars/cle-cliproxy");
    let output_dir = sidecar_dir.join("bin");

    println!("cargo:rerun-if-env-changed=CLE_SKIP_CLIPROXY_BUILD");
    emit_sidecar_rerun_inputs(&sidecar_dir);
    std::fs::create_dir_all(&output_dir).expect("failed to create cle-cliproxy bin dir");
