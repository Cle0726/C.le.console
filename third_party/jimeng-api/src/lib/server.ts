...
class Server {
    ...
    constructor() {
        ...
        // 前置处理异常拦截
        this.app.use(async (ctx: any, next: import('koa').Next) => {
        ...
        this.app.use(async (ctx: any, next: import('koa').Next) => {
            if (ctx.is('application/json') && ['POST', 'PUT', 'PATCH'].includes(ctx.method)) {
                ...
                // 空 JSON 请求体在健康检查、账号查询等 POST 端点中等价于 `{}`。
                // 旧客户端会发送 Content-Type: application/json + Content-Length: 0，
                // 不能把这种兼容请求误判成损坏的 JSON。
                let parsedBody;
