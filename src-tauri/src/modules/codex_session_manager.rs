...
const SESSION_TRASH_ROOT_DIR: &str = "cle-console-codex-session-trash";
...
fn read_session_export_manifest(
    ...
    if manifest.kind != SESSION_EXPORT_KIND {
        return Err("这不是 C.le.控制台 Codex 会话包".to_string());
    }
...
fn write_imported_rollout_from_archive(
    ...
    let temp_path = parent.join(format!(".cle-session-import-{}.tmp", Uuid::new_v4()));
