...
/// Tracks the main window minimize edge so a resize burst only opens one status window.
static MAIN_WINDOW_MINIMIZED: AtomicBool = AtomicBool::new(false);

/// Suppresses minimize-edge handling while the compact window restores the main window.
/// Windows can deliver focus/resize notifications out of order during this transition.
static MAIN_WINDOW_RESTORING: AtomicBool = AtomicBool::new(false);

/// Generation token for delayed restore verification. A stale verifier must never
/// release the guard belonging to a newer restore attempt.
static MAIN_WINDOW_RESTORE_EPOCH: AtomicU64 = AtomicU64::new(0);
...
#[cfg(test)]
mod tests {
    use super::{should_hide_startup_minimized_window, should_show_status_window};
    ...
    #[test]
    fn status_window_only_opens_on_main_window_minimize_edge() {
        assert!(should_show_status_window(false, true, false));
        assert!(!should_show_status_window(true, true, false));
        assert!(!should_show_status_window(true, false, false));
        assert!(!should_show_status_window(false, false, false));
        assert!(!should_show_status_window(false, true, true));
    }
}
...
pub(crate) fn finish_main_window_restore(epoch: u64, restored: bool) -> bool {
    if MAIN_WINDOW_RESTORE_EPOCH.load(Ordering::SeqCst) != epoch {
        return false;
    }
    if restored {
        MAIN_WINDOW_MINIMIZED.store(false, Ordering::SeqCst);
    }
    MAIN_WINDOW_RESTORING.store(false, Ordering::SeqCst);
    true
}

pub(crate) fn is_main_window_restore_current(epoch: u64) -> bool {
    MAIN_WINDOW_RESTORING.load(Ordering::SeqCst)
        && MAIN_WINDOW_RESTORE_EPOCH.load(Ordering::SeqCst) == epoch
}
...
pub fn run() {
    ...
    let app = tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            modules::doubao_web::ensure_keepalive_started(app.handle().clone());
        ...
        .on_window_event(|window, event| match event {
            WindowEvent::Resized(_) => {
                ...
                // the edge baseline; a delayed verifier releases this guard once the main
                // window has remained visible and restored.
                if MAIN_WINDOW_RESTORING.load(Ordering::SeqCst) {
                    return;
                }
                ...
                match window.is_minimized() {
                    Ok(is_minimized) => {
                        let was_minimized =
                            MAIN_WINDOW_MINIMIZED.swap(is_minimized, Ordering::SeqCst);
                        if should_show_status_window(was_minimized, is_minimized, false) {
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::system::get_network_egress_snapshot,
            ...
            commands::system::show_status_window,
            commands::system::hide_status_window,
            ...
            commands::codex::get_codex_unfinished_session,
            commands::codex::dismiss_codex_unfinished_session,
            ...
            commands::jimeng_api::doubao_credentials_export_file,
            commands::jimeng_api::doubao_credentials_import_file,
            ...
            commands::jimeng_api::web_creator_collect_assets,
            commands::jimeng_api::web_creator_clear_assets,
            commands::jimeng_api::web_creator_download_asset,
