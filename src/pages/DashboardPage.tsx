...
import { GitHubCopilotAccount, getGitHubCopilotUsage } from '../types/githubCopilot';
import {
  WindsurfAccount,
...
import { SpatialCommandHero } from '../components/SpatialCommandHero';
import {
  DashboardLaunchpad,
  DashboardModelGallery,
  DashboardSceneHeader,
  type DashboardScene,
} from '../components/DashboardShowcase';
import { TrafficEgressScene } from '../components/TrafficEgressScene';
...
function clampDashboardPercentage(value: number): number {
  return Math.max(0, Math.min(100, value));
}

function pickTightestRemainingPercentage(
  values: Array<number | null | undefined>,
): number | null {
  const normalized = values
    .filter((value): value is number => typeof value === 'number' && Number.isFinite(value))
    .map(clampDashboardPercentage);
...
export function DashboardPage({
  ...
  const [dashboardScene, setDashboardScene] = React.useState<DashboardScene>('landing');
  const [selectedModelId, setSelectedModelId] = React.useState('gpt');
  ...
  const visibleEntryOrder = useMemo(
    () =>
      orderedEntryIds.filter((entryId) => {
        return !hiddenEntrySet.has(entryId)
          && resolveEntryPlatformIds(entryId, platformGroups).length > 0;
      }),
    [orderedEntryIds, hiddenEntrySet, platformGroups],
  );
  const visibleDashboardEntryOrder = visibleEntryOrder as DashboardEntryId[];
  ...
  const navigateToPlatform = useCallback((platformId: PlatformId) => {
    if (platformId === 'chat2api') {
      invoke('start_chat2api').catch(console.error);
      return;
    }
    if (platformId === 'aurora') {
      invoke('start_aurora').catch(console.error);
      return;
    }
  ...
  const currentModelQuota = useMemo(() => {
    const modelLabel = selectedModelId === 'claude'
      ? 'Claude'
    ...
    if (selectedModelId === 'gpt' || selectedModelId === 'codex') {
      const quotaItems = codexCurrentAccount
        ? buildCodexAccountPresentation(codexCurrentAccount, t).quotaItems.filter((item) =>
          item.key === 'primary' || item.key === 'secondary' || item.key === 'new_api_quota',
        )
        : [];
      remainingPercent = pickTightestRemainingPercentage(
        quotaItems.map((item) => item.percentage),
      );
    } else if (selectedModelId === 'claude') {
      const used = [
        claudeCurrent?.quota?.five_hour_percentage,
        claudeCurrent?.quota?.seven_day_percentage,
      ].filter((value): value is number => typeof value === 'number' && Number.isFinite(value));
      remainingPercent = used.length > 0
        ? clampDashboardPercentage(100 - Math.max(...used))
        : null;
    } else if (selectedModelId === 'gemini') {
      const tiers = geminiCurrent ? getGeminiTierQuotaSummary(geminiCurrent) : null;
      remainingPercent = pickTightestRemainingPercentage([
        tiers?.gemini5h.remainingPercent,
        tiers?.geminiWeekly.remainingPercent,
      ]);
    } else if (selectedModelId === 'antigravity') {
      remainingPercent = pickTightestRemainingPercentage(
        agCurrentAccount?.quota?.models?.map((model) => model.percentage) ?? [],
      );
    } else if (selectedModelId === 'github' && githubCopilotCurrent) {
      const usage = getGitHubCopilotUsage(githubCopilotCurrent);
      remainingPercent = pickTightestRemainingPercentage([
        usage.inlineIncluded
          ? 100
          : usage.inlineSuggestionsUsedPercent == null
            ? null
            : 100 - usage.inlineSuggestionsUsedPercent,
        usage.chatIncluded
          ? 100
          : usage.chatMessagesUsedPercent == null
            ? null
            : 100 - usage.chatMessagesUsedPercent,
        usage.premiumIncluded
          ? 100
          : usage.premiumRequestsUsedPercent == null
            ? null
            : 100 - usage.premiumRequestsUsedPercent,
      ]);
    }

    return { modelLabel, remainingPercent };
  }, [
    agCurrentAccount,
    claudeCurrent,
    codexCurrentAccount,
  ...
  const renderCodexAccountContent = (account: CodexAccount | null) => {
    ...
    if (isCodexApiKeyAccount(account) && !isCodexNewApiAccount(account)) {
      ...
      return (
        <div className="account-mini-card codex-api-mini-card">
          <div className="account-mini-header">
            <div className="account-info-row">
            ...
            <div className="account-mini-quotas codex-api-mini-quotas">
              {usageMode === 'new_api' ? (
                <div className="mini-quota-row-stacked">
                  <div className="mini-quota-header">
                    <span className="model-name">{t('codex.cleApi.balance', '额度')}</span>
  ...
  const visibleDashboardCardIds = useMemo(() => {
    ...
    for (const entryId of visibleDashboardEntryOrder) {
      const entryPlatformIds = resolveEntryPlatformIds(entryId, platformGroups);
  ...
  if (dashboardScene === 'landing') {
    return (
      <DashboardLaunchpad
        modelCount={6}
        quotaPercent={currentModelQuota.remainingPercent}
        quotaModelLabel={currentModelQuota.modelLabel}
        onOpenModels={() => setDashboardScene('models')}
        onOpenData={() => setDashboardScene('data')}
        onOpenEgress={() => setDashboardScene('egress')}
        onModelChange={setSelectedModelId}
      />
    );
  }

  if (dashboardScene === 'models') {
    return (
      <DashboardModelGallery
        onBack={() => setDashboardScene('landing')}
        onSelect={(modelId) => {
          setSelectedModelId(modelId);
          setDashboardScene('studio');
        }}
      />
    );
  }

  if (dashboardScene === 'studio') {
    return (
      <main className="main-content dashboard-scene model-studio-page fade-in">
        <DashboardSceneHeader title="模型工作台" englishTitle="MODEL STUDIO" index="02" onBack={() => setDashboardScene('models')} />
        <SpatialCommandHero
          totalAccounts={stats.total}
          activePlatforms={visibleDashboardEntryOrder.length}
  ...
  if (dashboardScene === 'egress') {
    return <TrafficEgressScene onBack={() => setDashboardScene('landing')} />;
  }

  return (
    <main className="main-content dashboard-page fade-in">
      <div className="dashboard-data-scene-header">
        <DashboardSceneHeader title="账号总览" englishTitle="ACCOUNT OVERVIEW" index="03" onBack={() => setDashboardScene('landing')} />
        <button className="scene-layout-button" onClick={onOpenPlatformLayout}>
          <span>{t('platformLayout.title', '平台布局')}</span><small>PLATFORM LAYOUT</small>
        </button>
      </div>
