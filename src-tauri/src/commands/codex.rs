...
pub async fn fetch_codex_account_note_mail_url(
    ...
    let client = reqwest::Client::builder()
        ...
        .user_agent("CleConsole-MailPreview/1.0")
        .build()
...
pub async fn codex_query_model_provider_usage(
    ...
    let resolved_base_url = if base_url.contains("claude.ai") {
        "http://127.0.0.1:8766/api".to_string()
    } else {
        base_url
    };
    ...
    match requested_type {
        ...
        None => {
            let new_api_error =
                match query_new_api_model_provider_usage(&client, &resolved_base_url, key).await {
                    Ok(summary) => return Ok(summary),
                    Err(error) => error,
                };
            match query_sub2api_model_provider_usage(&client, &resolved_base_url, key).await {
