...
const CLE_API_PROVIDER_ID: &str = "cle_api";
const LEGACY_NEW_API_PROVIDER_ID: &str = "new_api";
const CLE_API_PLAN_TYPE: &str = "Cle Api";
const LEGACY_NEW_API_EXCLUSIVE_PLAN_TYPE: &str = "NEW_API_EXCLUSIVE";
const CLE_API_BASE_URL: &str = "https://chongcodex.cn/v1";
...
fn is_new_api_account(account: &CodexAccount) -> bool {
    account
        ...
        .map(|value| {
            ...
            value.eq_ignore_ascii_case(CLE_API_PROVIDER_ID)
                || value.eq_ignore_ascii_case(LEGACY_NEW_API_PROVIDER_ID)
        })
        .unwrap_or(false)
        || is_cle_api_base_url(account.api_base_url.as_deref())
        || account
            .plan_type
            .as_deref()
            .map(|value| {
                let value = value.trim();
                value.eq_ignore_ascii_case(CLE_API_PLAN_TYPE)
...
fn is_cle_api_base_url(raw: Option<&str>) -> bool {
    ...
    let Some(expected) = normalize_api_base_url_for_match(Some(CLE_API_BASE_URL)) else {
        return false;
    };
    actual == expected
}
...
fn build_new_api_profile_url(account: &CodexAccount) -> Result<String, String> {
    ...
    parsed.set_path("/api/cle-console/token-profile");
...
async fn fetch_new_api_quota(account: &CodexAccount) -> Result<FetchQuotaResult, String> {
    let api_key = account
        ...
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .ok_or("Cle Api 账号缺少 OPENAI_API_KEY")?;
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|err| format!("请求 Cle Api 额度失败: {}", err))?;
    let status = response.status();
    let body = response
        .text()
        .await
        .map_err(|err| format!("读取 Cle Api 额度响应失败: {}", err))?;
    if !status.is_success() {
        return Err(format!("Cle Api 额度接口返回 HTTP {}", status.as_u16()));
    }

    let root: serde_json::Value = serde_json::from_str(&body)
        .map_err(|err| format!("解析 Cle Api 额度 JSON 失败: {}", err))?;
    if root.get("success").and_then(|item| item.as_bool()) == Some(false) {
        let message = root
            .get("message")
            .and_then(|item| item.as_str())
            .unwrap_or("Cle Api 额度接口返回失败");
        return Err(message.to_string());
    }
    let data = root.get("data").unwrap_or(&root);
    let usage = data.get("usage").ok_or("Cle Api 额度响应缺少 usage")?;
    ...
    Ok(FetchQuotaResult {
        ...
        plan_type: Some(
            data.get("plan_type")
                ...
                .filter(|value| !value.is_empty())
                .unwrap_or_else(|| CLE_API_PLAN_TYPE.to_string()),
        ),
    })
}
            ...
            let result = match fetch_new_api_quota(&account).await {
                ...
                Err(e) => {
                    ...
                    if let Err(save_err) = codex_account::save_account(&account) {
                        logger::log_warn(&format!("写入 Cle Api 配额错误失败: {}", save_err));
                    }
                    return Err(e);
                }
            };
