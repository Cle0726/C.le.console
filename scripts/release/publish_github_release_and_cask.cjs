...
const DEFAULT_REPO = 'Cle0726/C.le.console';
const DEFAULT_CASK_PATH = 'Casks/cle-console.rb';
const DEFAULT_TARGET = 'universal-apple-darwin';
const DEFAULT_RELEASE_ASSET_PREFIX = 'Cle.Tools';
...
function printHelp() {
  ...
  console.log('  --repo <owner/repo>     GitHub repo for release (default: Cle0726/C.le.console)');
  console.log('  --cask <path>           Homebrew cask file path (default: Casks/cle-console.rb)');
...
function resolveSourceDmgPath(version, options) {
  ...
  const defaultPath = path.resolve(
    ...
    'bundle',
    'dmg',
    `C.le.控制台_${version}_universal.dmg`
  );
...
function main() {
  ...
  console.log('C.le.控制台 GitHub Release + Homebrew cask publisher');
  ...
  console.log('- 检查 Casks/cle-console.rb diff');
