...
/// WorkBuddy 按北京时间结算每日活动。固定使用 UTC+8，避免电脑系统时区或夏令时
/// 让账号跨错日期、提前或延后签到。
fn beijing_offset() -> FixedOffset {
    FixedOffset::east_opt(8 * 60 * 60).expect("UTC+8 must be a valid fixed offset")
}
...
fn add_log_record(record: WorkbuddyAutoCheckinLogRecord) -> Result<(), String> {
    ...
    let cutoff = beijing_now().timestamp() - THIRTY_DAYS_SECS;

    logs.retain(|r| {
        if let Ok(ndt) = chrono::NaiveDateTime::parse_from_str(&r.timestamp, "%Y-%m-%d %H:%M:%S") {
            if let Some(beijing_dt) = beijing_offset().from_local_datetime(&ndt).single() {
                beijing_dt.timestamp() >= cutoff
            } else {
...
pub async fn run_workbuddy_auto_checkin_cycle_if_needed(
    ...
    if accounts.is_empty() {
        if force {
            add_log_record(WorkbuddyAutoCheckinLogRecord {
                id: format!(
                    "log_{}_{}",
                    beijing_now().timestamp_millis(),
                    rand::random::<u16>()
                ),
                timestamp: beijing_now().format("%Y-%m-%d %H:%M:%S").to_string(),
    ...
    let start_timestamp_str = beijing_now().format("%Y-%m-%d %H:%M:%S").to_string();
