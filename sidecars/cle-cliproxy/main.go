...
const (
	clientAPIKeyContextKey contextKey = "cleClientAPIKey"
	requestKindContextKey  contextKey = "cleRequestKind"
	requestModelContextKey contextKey = "cleRequestModel"
)
...
func buildOllamaShowResponse(model string, modifiedAt time.Time) gin.H {
	...
	return gin.H{
		"model":        model,
		"remote_model": model,
		"license":      "Proxied via Cle Codex API Service",
...
func (s *cleSelector) orderCustom(auths []*coreauth.Auth, start int) []*coreauth.Auth {
...
func weightedOrder(group []*coreauth.Auth, rules map[string]customRoutingRule, selector *cleSelector, start int) []*coreauth.Auth {
	if len(group) <= 1 {
		return group
	}
	total := 0
...
func (s *cleSelector) accountForAuth(auth *coreauth.Auth) *accountSpec {
...
func (s *cleSelector) emitAuthSelected(ctx context.Context, auth *coreauth.Auth, provider, model string, candidateAuths, availableAuths int) {
...
func (s *cleSelector) rotatedIndex(account *accountSpec, start int) int {
...
func main() {
	...
	manifestPath := flag.String("manifest", "", "Cle sidecar manifest file")
	parentPID := flag.Int("parent-pid", 0, "C.le.控制台 parent process id")
	flag.Parse()
	...
	selector := &cleSelector{manifest: m, emitter: emitter}
