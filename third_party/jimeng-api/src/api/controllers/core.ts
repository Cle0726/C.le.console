...
export async function getCredit(refreshToken: string) {
  ...
  const {
    ...
    // The current commerce endpoint rejects the generic AIGC query string.
    // Match the browser request and the upstream v1.9.7 implementation.
...
export async function receiveCredit(refreshToken: string) {
  ...
  const timeZone = regionInfo.isJP
    ? "Asia/Tokyo"
    : regionInfo.isUS
      ? "America/Los_Angeles"
      : regionInfo.isHK
...
export async function request(
  ...
  uri: string,
  refreshToken: string,
  options: AxiosRequestConfig & { noDefaultParams?: boolean; maxRetries?: number } = {}
) {
  ...
  const requestOrigin = regionInfo.isInternational
    ? "https://dreamina.capcut.com"
    : origin;
  const defaultReferer = regionInfo.isInternational
    ? "https://dreamina.capcut.com/"
    : `${origin}/`;
  ...
  const headers = {
    ...
    "Sec-Fetch-Site": regionInfo.isInternational ? "same-site" : "same-origin",
  ...
  while (retries <= maxRetries) {
    try {
      ...
      const response = await axios.request({
        ...
        ..._.omit(options, "params", "headers", "noDefaultParams", "maxRetries"),
 ...
 * 获取Token存活状态
 */
function containsAccountIdentity(value: unknown, depth = 0): boolean {
  if (!value || depth > 6) return false;
  if (Array.isArray(value)) {
    return value.some((item) => containsAccountIdentity(item, depth + 1));
  }
  if (typeof value !== "object") return false;
  const record = value as Record<string, unknown>;
  const identityKeys = ["user_id", "userId", "uid", "sec_user_id", "account_id"];
  ...
  return Object.values(record).some((item) => containsAccountIdentity(item, depth + 1));
}
...
export async function getTokenLiveStatus(refreshToken: string) {
  try {
    const { token: tokenWithRegion, proxyUrl } = parseProxyFromToken(refreshToken);
    const regionInfo = parseRegionFromToken(tokenWithRegion);
    const home = regionInfo.isInternational
    ...
    if (response.status >= 400 || typeof response.data !== "string") return false;
    if (/window\.__isLogined\s*=\s*true/i.test(response.data)) return true;
    const marker = "window.__userInfoStringify";
    const markerIndex = response.data.indexOf(marker);
    ...
    return payload?.errmsg === "success" && containsAccountIdentity(payload?.data);
  } catch (err) {
    logger.debug(`网页身份标记检查失败: ${err instanceof Error ? err.message : String(err)}`);
    return false;
  }
}
