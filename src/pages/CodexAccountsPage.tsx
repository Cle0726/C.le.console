...
import {
  ...
  CLE_API_BASE_URL,
  CLE_API_PROVIDER_ID,
  CLE_API_PROVIDER_NAME,
  findCodexApiProviderPresetById,
  isCleApiProviderBaseUrl,
...
const CODEX_BATCH_IMPORT_SESSION_STORAGE_KEY =
  "cle.codex.batchImport.sessionId";
...
type CleApiJsonRecord = Record<string, unknown>;

function toCleApiRecord(value: unknown): CleApiJsonRecord | null {
  return value && typeof value === "object" && !Array.isArray(value)
    ? (value as CleApiJsonRecord)
    : null;
}

function readCleApiString(
  record: CleApiJsonRecord | null,
  key: string,
): string {
...
function readCleApiOptionalNumber(
  record: CleApiJsonRecord | null,
  key: string,
): number | null {
  const value = record?.[key];
...
function formatCleApiInteger(value: number): string {
  return new Intl.NumberFormat("en-US", { maximumFractionDigits: 0 }).format(
    Math.max(0, value),
  );
}

function formatCleApiTokenCount(value: number): string {
...
function getCleApiUsageRecord(
  account: CodexAccount,
): CleApiJsonRecord | null {
  const raw = toCleApiRecord(account.quota?.raw_data);
  const profile = toCleApiRecord(raw?.profile);
  return toCleApiRecord(raw?.usage) ?? toCleApiRecord(profile?.usage);
}

function getCleApiStatsRecord(
  account: CodexAccount,
): CleApiJsonRecord | null {
  const usage = getCleApiUsageRecord(account);
  return toCleApiRecord(usage?.stats);
}
...
function AccountTableHeading({
  ...
  return (
    <span className="account-table-heading">
      <span className="account-table-heading-zh">{zh}</span>
      <span className="account-table-heading-en">{en}</span>
    </span>
  );
}
...
export function CodexAccountsPage() {
  ...
  const [exportFormat, setExportFormat] =
    useState<CodexExportFormat>("cle_console");
  ...
  const [cliLaunchingAccountId, setCliLaunchingAccountId] = useState<
    string | null
  >(null);
  const [cleApiPanelAccountId, setCleApiPanelAccountId] = useState<
    string | null
  >(null);
  ...
  const exportFormatOptions = useMemo<SingleSelectFilterOption[]>(
    () => [
      {
        value: "cle_console",
        label: t("codex.exportFormat.cleConsole", "C.le.控制台"),
      },
      {
        value: "sub2api",
  ...
  const defaultApiProviderPresetId = DEFAULT_CODEX_API_PROVIDER_ID;
  ...
  const cleApiPanelAccount = useMemo(
    () =>
      cleApiPanelAccountId
        ? (accounts.find((item) => item.id === cleApiPanelAccountId) ??
          null)
        : null,
    [accounts, cleApiPanelAccountId],
  );
  ...
  useEffect(() => {
    if (cleApiPanelAccountId && !cleApiPanelAccount) {
      setCleApiPanelAccountId(null);
    }
  }, [cleApiPanelAccount, cleApiPanelAccountId]);
  ...
  const buildApiProviderPayload = useCallback(
    (
      ...
      if (isCleApiProviderBaseUrl(normalizedBaseUrl)) {
        return {
          apiProviderMode: "custom",
          apiProviderId: CLE_API_PROVIDER_ID,
          apiProviderName: CLE_API_PROVIDER_NAME,
        };
      }
      ...
      const customProviderDisplayName = trimmedName || undefined;
      return {
        apiProviderMode: "custom",
  ...
  useEffect(() => {
    if (!showAddModal) {
      const defaultProvider = resolveApiProviderPresetDefaults(defaultApiProviderPresetId);
      setApiKeyInput("");
  ...
  const handleApiKeyLogin = async () => {
    ...
    try {
      ...
      if (
        ...
        providerPayload.apiProviderId !== CLE_API_PROVIDER_ID
      ) {
        try {
  ...
  const resolveCleApiAccountBalanceText = useCallback(
    (account: CodexAccount): string | null => {
      const usage = getCleApiUsageRecord(account);
      const stats = getCleApiStatsRecord(account);
      const total = toCleApiRecord(stats?.total);
      const profile = toCleApiRecord(
        toCleApiRecord(account.quota?.raw_data)?.profile,
      );
      const records = [usage, total, profile].filter(
        (record): record is CleApiJsonRecord => Boolean(record),
      );
      const displayKeys = [
        "balance_display",
      ...
      for (const record of records) {
        for (const key of displayKeys) {
          const value = readCleApiString(record, key);
      ...
      for (const record of records) {
        for (const key of numberKeys) {
          const value = readCleApiOptionalNumber(record, key);
  ...
  const formatApiKeyUsageDetailValue = useCallback(
    (item: { key: string; value: string }, unit?: string | null): string => {
      ...
      if (
        ...
        return formatCleApiTokenCount(numeric);
      }
      ...
      if (
        ...
        return formatCleApiInteger(numeric);
      }
      ...
      if (variant === "card" && summary && isNewApiUsage) {
        ...
        return (
          <div
            ...
            title={`${t("codex.cleApi.balance", "额度")}：${quotaValueText}`}
          >
            <div className="quota-header">
              <Database size={14} />
              <span className="quota-label">
                {t("codex.cleApi.balance", "额度")}
              </span>
          ...
          <div className="codex-api-key-usage-panel sub2api">
            <div className="codex-api-key-usage-grid">
              ...
              <div>
                ...
                <strong>
                  {formatCleApiInteger(summary.todayRequests ?? 0)}
                </strong>
              </div>
              <div>
                <span>
                ...
                <strong>
                  {formatCleApiTokenCount(summary.todayTotalTokens ?? 0)}
                </strong>
              </div>
            </div>
          </div>
            ...
            <>
              ...
              {isNewApiUsage ? (
                <div className="codex-api-key-usage-progress">
                  <div className="cle-api-progress-track">
                    <div
                      className="cle-api-progress-bar"
  ...
  const renderGridCards = (items: typeof filteredAccounts, groupKey?: string) =>
    items.map((account) => {
      ...
      const displayPlanClass = isQuotaAwareApiKeyAccount
        ? "new-api-exclusive"
        : planClass;
      const displayPlanLabel = presentation.planLabel;
      const cleApiAccountBalanceText = isNewApiAccount
        ? resolveCleApiAccountBalanceText(account)
        : null;
      ...
      return (
        <div
          key={groupKey ? `${groupKey}-${account.id}` : account.id}
          className={`codex-account-card ${isCurrent ? "current" : ""} ${isSelected ? "selected" : ""} ${isPendingOAuthAccount ? "pending-auth" : ""} ${isNewApiAccount ? "new-api-exclusive" : ""} ${isQuotaAwareApiKeyAccount ? "api-key-usage-account" : ""}`}
        >
              ...
              <>
                ...
                {cleApiAccountBalanceText && (
                  <div className="codex-account-balance-line">
                    ...
                    <strong>{cleApiAccountBalanceText}</strong>
                  </div>
                )}
                {quotaItems.map((item) => {
                  const QuotaIcon =
                ...
                {quotaItems.length === 0 && !cleApiAccountBalanceText && (
            ...
            <div className="card-footer">
              <div className="card-actions">
                ...
                {isNewApiAccount && (
                  <button
                    className="card-action-btn"
                    onClick={() => setCleApiPanelAccountId(account.id)}
                    title={t("codex.cleApi.servicePanel", "服务面板")}
                  >
                    <Database size={14} />
                  </button>
                )}
  ...
  const renderTableRows = (items: typeof filteredAccounts, groupKey?: string) =>
    items.map((account) => {
      ...
      return (
        <tr
          key={groupKey ? `${groupKey}-${account.id}` : account.id}
          className={`${isCurrent ? "current" : ""} ${isPendingOAuthAccount ? "pending-auth" : ""} ${isNewApiAccount ? "new-api-exclusive" : ""} ${isQuotaAwareApiKeyAccount ? "api-key-usage-account" : ""}`}
        >
          <td>
            <input
              type="checkbox"
  ...
  const renderApiKeyUsageDetailModal = () => {
        ...
        : usageMode === "sub2api"
          ? [
              ...
              },
              {
                ...
                value: formatCleApiInteger(summary.todayRequests ?? 0),
              },
              {
                key: "todayTokens",
                label: t(
    ...
    const summaryGridClassName =
      usageMode === "sub2api" || usageMode === "new_api"
        ? "cle-api-summary-grid compact"
        : "cle-api-summary-grid";
    ...
    return (
      ...
      >
        <div
          className="modal-content cle-api-panel-modal codex-api-key-usage-detail-modal"
          onClick={(event) => event.stopPropagation()}
        >
          <div className="modal-header cle-api-panel-header">
            <div>
              <h2>{t("codex.modelProviders.usage.detailTitle", "服务面板")}</h2>
              <span className="cle-api-panel-subtitle">
              ...
              {summaryDetails.map((item) => (
                ...
                >
                  ...
                  {(item.key === "remaining" ||
                    ...
                    usageMode !== "sub2api" && (
                      <div>
                        <div className="cle-api-progress-row">
                          <div className="cle-api-progress-track">
                            <div
                              className="cle-api-progress-bar"
                              style={{ width: `${usedPercent}%` }}
                ...
                {details.length > 0 ? (
                  details.map((item) => (
                    <div className="cle-api-usage-card" key={item.key}>
                      <span className="cle-api-card-label">
  ...
  const renderCleApiServicePanel = () => {
    const account = cleApiPanelAccount;
    if (!account) return null;

    const usage = getCleApiUsageRecord(account);
    const stats = getCleApiStatsRecord(account);
    const requests = toCleApiRecord(stats?.requests);
    const tokens = toCleApiRecord(stats?.tokens);
    const total = toCleApiRecord(stats?.total);
    const modelItems = (Array.isArray(stats?.models) ? stats.models : [])
      .map(toCleApiRecord)
      .filter((item): item is CleApiJsonRecord => Boolean(item))
      .slice(0, 8);
    ...
    const visible = visibleApiKeyAccountIds.has(account.id);
    const apiKeyDisplay = resolveApiKeyDisplayText(account, visible);
    const baseUrl = (account.api_base_url || "").trim() || CLE_API_BASE_URL;
    const quotaText = readCleApiString(usage, "summary_display") || "-";
    const cleApiAccountBalanceText =
      resolveCleApiAccountBalanceText(account);
    const usedPercent = readCleApiNumber(usage, "used_percent");
    const requestCount = readCleApiNumber(requests, "total");
    const todayCount = readCleApiNumber(requests, "today");
    const last7Count = readCleApiNumber(requests, "last_7_days");
    const last30Count = readCleApiNumber(requests, "last_30_days");
    const totalTokens = readCleApiNumber(tokens, "total");
    const totalQuotaDisplay = readCleApiString(total, "quota_display");
    const panelDisplayName = resolvePresentation(account).displayName;
    const hasStats = Boolean(stats);
    const usedPercentText = `${formatCleApiInteger(usedPercent)}%`;
    const summaryItems = [
      {
        key: "requests",
        label: t("codex.cleApi.requests", "请求"),
        value: formatCleApiInteger(requestCount),
        meta: `${t("codex.cleApi.today", "今日")} ${formatCleApiInteger(todayCount)}`,
      },
      {
        key: "periods",
        label: t("codex.cleApi.periods", "周期"),
        value: `7d ${formatCleApiInteger(last7Count)}`,
        meta: `30d ${formatCleApiInteger(last30Count)}`,
      },
      {
        key: "tokens",
        label: t("codex.cleApi.tokens", "Tokens"),
        value: formatCleApiTokenCount(totalTokens),
        meta: `${t("codex.cleApi.quotaUsed", "消耗")} ${totalQuotaDisplay || "-"}`,
      },
    ];
      ...
      >
        ...
        >
          <div className="modal-header cle-api-panel-header">
            <div>
              <h2>
                {t("codex.cleApi.panelTitle", "C.le. API 服务面板")}
              </h2>
              <span className="cle-api-panel-subtitle">
            ...
            <button
              className="modal-close"
              onClick={() => setCleApiPanelAccountId(null)}
              aria-label={t("common.close", "关闭")}
            >
              <X />
            </button>
          ...
          <div className="cle-api-panel-body">
            ...
            <section className="cle-api-summary-grid">
              <div className="cle-api-balance-card">
                <span className="cle-api-card-label">
                  {t("codex.cleApi.balance", "额度")}
                </span>
                <strong>{quotaText}</strong>
                {cleApiAccountBalanceText && (
                  <small className="cle-api-balance-meta">
                    {t("codex.modelProviders.usage.accountBalance", "账户余额")}
                    ：{cleApiAccountBalanceText}
                  </small>
                )}
              ...
              {summaryItems.map((item) => (
                <div className="cle-api-stat-card" key={item.key}>
                  <span className="cle-api-card-label">{item.label}</span>
                  <strong>{item.value}</strong>
            ...
            {hasStats ? (
              <div className="cle-api-stats-grid">
                <section className="cle-api-panel-section">
                  <div className="cle-api-section-head">
                    <strong>
                      {t("codex.cleApi.modelStats", "模型统计")}
                    </strong>
                  </div>
                  <div className="cle-api-usage-list">
                    {modelItems.length > 0 ? (
                      modelItems.map((item) => {
                        const modelName =
                          readCleApiString(item, "model_name") || "-";
                        const count = readCleApiNumber(
                          item,
                          "request_count",
                        );
                        const modelTokens = readCleApiNumber(
                          item,
                          "total_tokens",
                        );
                        const quotaDisplay = readCleApiString(
                          item,
                          "quota_display",
                        );
                        return (
                          <div
                            className="cle-api-usage-row"
                            key={modelName}
                          >
                            <div>
                              <span className="cle-api-usage-name">
                                {modelName}
                              </span>
                              <small>
                                {t("codex.cleApi.requests", "请求")}{" "}
                                {formatCleApiInteger(count)}
                              </small>
                            </div>
                            <div className="cle-api-usage-values">
                              <span>
                                {t("codex.cleApi.tokens", "Tokens")}{" "}
                                {formatCleApiTokenCount(modelTokens)}
                              </span>
                ...
                <section className="cle-api-panel-section">
                  <div className="cle-api-section-head">
                    <strong>
                      {t("codex.cleApi.dailyStats", "每日统计")}
                    </strong>
                  </div>
                  <div className="cle-api-usage-list">
                    {dailyItems.length > 0 ? (
                      dailyItems.map((item) => {
                        const date = readCleApiString(item, "date") || "-";
                        const count = readCleApiNumber(
                          item,
                          "request_count",
                        );
                        const dayTokens = readCleApiNumber(
                          item,
                          "total_tokens",
                        );
                        const quotaDisplay = readCleApiString(
                        ...
                        return (
                          <div className="cle-api-usage-row" key={date}>
                            <div>
                              <span className="cle-api-usage-name">
                                {date}
                              </span>
                            ...
                            <div className="cle-api-usage-values">
                              <span>
                                {t("codex.cleApi.tokens", "Tokens")}{" "}
                                {formatCleApiTokenCount(dayTokens)}
                              </span>
                              <strong>{quotaDisplay || "-"}</strong>
                            </div>
                          </div>
              ...
              <div className="cle-api-empty-state">
                {t(
                  "codex.cleApi.refreshHint",
    ...
    >
      ...
      {activeTab === "overview" && (
        <>
          ...
          <div
            className="toolbar account-command-dock"
            role="toolbar"
            aria-label="账号筛选与操作 / Account filters and actions"
          >
            <div className="toolbar-left" role="group" aria-label="查找与整理 / Find and organize">
              <span className="account-toolbar-kicker" aria-hidden="true">
                <strong>查找与整理</strong>
                <small>FIND &amp; ORGANIZE</small>
              </span>
              <div className="search-box">
                <Search size={16} className="search-icon" />
                <input
                  type="text"
                  placeholder="搜索账号 / Search accounts..."
                  aria-label="搜索账号 / Search accounts"
                  value={searchQuery}
              ...
              <div className="view-switcher">
                <button
                  ...
                  title="紧凑视图 / Compact view"
                  aria-label="紧凑视图 / Compact view"
                >
                  <Rows3 size={16} />
                </button>
                <button
                ...
                <button
                  ...
                  title="卡片视图 / Card view"
                  aria-label="卡片视图 / Card view"
                >
                  <LayoutGrid size={16} />
                </button>
              </div>
              <MultiSelectFilterDropdown
                ...
                allLabel={`全部 (${tierCounts.all}) / All`}
                filterLabel="筛选 / Filter"
                clearLabel="清空筛选 / Clear"
                emptyLabel="暂无 / None"
                ariaLabel="账户筛选 / Account filter"
              ...
              <div className="tag-filter" ref={tagFilterRef}>
                <button
                  ...
                  aria-label="标签筛选 / Tag filter"
                >
                  <Tag size={14} />
                  {tagFilter.length > 0
                    ? `标签 ${tagFilter.length} / Tags`
                    : "标签 / Tags"}
                </button>
                {showTagFilter && (
                  <div
              ...
              <SingleSelectFilterDropdown
                ...
                options={[
                  ...
                  {
                    value: "weekly",
                    label: "周额度 / Weekly quota",
                  },
                  {
                    value: "hourly",
                    label: "5 小时额度 / 5h quota",
                  },
                  {
                    value: "weekly_reset",
                    label: "周额度重置 / Weekly reset",
                  },
                  {
                    value: "hourly_reset",
                    label: "5 小时重置 / 5h reset",
                  },
                  {
              ...
              {!isCustomSortActive && (
                <button
                  ...
                  title={
                    sortDirection === "desc"
                      ? "当前降序 · 切换为升序 / Descending · switch to ascending"
                      : "当前升序 · 切换为降序 / Ascending · switch to descending"
                  }
                  aria-label="切换排序方向 / Toggle sort direction"
                >
                  {sortDirection === "desc" ? "⬇" : "⬆"}
                </button>
              )}
            </div>
            <div className="toolbar-right" role="group" aria-label="账号操作 / Account actions">
              <span className="account-toolbar-actions-label" aria-hidden="true">
                操作 <small>/ ACTIONS</small>
              </span>
              <button
                className="btn btn-primary icon-only"
                onClick={() => openCodexAddModal("oauth")}
                  title="添加账号 / Add account"
                  aria-label="添加账号 / Add account"
              >
                <Plus size={14} />
              </button>
              <button
                ...
                title="刷新全部 / Refresh all"
                aria-label="刷新全部 / Refresh all"
              >
                <RefreshCw
                  size={14}
              ...
              <button
                ...
                aria-label={privacyModeEnabled ? "显示邮箱 / Show details" : "隐藏邮箱 / Hide details"}
              >
              ...
              <button
                ...
                title={
                  exportSelectionCount > 0
                    ? `导出 / Export (${exportSelectionCount})`
                    : "导出 / Export"
                }
                aria-label="导出账号 / Export accounts"
              >
                <Upload size={14} />
              </button>
              {!activeGroupId && (
                <button
                  ...
                  title={
                    groupFilter.length > 0
                      ? `分组管理 / Groups (${groupFilter.length})`
                      : "分组管理 / Groups"
                  }
                  aria-label="分组管理 / Manage groups"
                >
                  <FolderOpen size={14} />
                </button>
              )}
                ...
                <div className="codex-overview-selection-bar">
                  <div className="codex-overview-selection-left">
                    <label className="codex-overview-select-all">
                      ...
                      />
                      <span>全选 <small>/ Select all</small></span>
                    </label>
                    ...
                    <table className="account-table">
                      <thead>
                        <tr>
                          ...
                          <th style={{ width: 260 }}>
                            <AccountTableHeading
                              zh={t("common.shared.columns.email", "账号")}
                              en="ACCOUNT"
                            />
                          </th>
                          <th style={{ width: 140 }}>
                            <AccountTableHeading
                              zh={t("common.shared.columns.plan", "订阅")}
                              en="PLAN"
                            />
                          </th>
                          <th style={{ width: 150 }}>
                            <AccountTableHeading
                              zh={t("codex.subscription.column", "订阅信息")}
                              en="SUBSCRIPTION"
                            />
                          </th>
                          <th>
                            <AccountTableHeading
                              zh={t("accounts.columns.quota", "配额状态")}
                              en="QUOTA STATUS"
                            />
                          </th>
                          <th className="sticky-action-header table-action-header">
                            <AccountTableHeading
                              zh={t("common.shared.columns.actions", "操作")}
                              en="ACTIONS"
                            />
                          </th>
                        </tr>
                ...
                <div className="modal-body">
                  ...
                  {addTab === "apikey" && (
                    <div className="add-section">
                      ...
                      {apiProviderPresetId !== CLE_API_PROVIDER_ID && (
                        <div className="oauth-link">
                          <label>
                            {t(
              ...
              >
                ...
                <div className="modal-body">
                  <div className="add-section">
                    ...
                    {editingApiProviderPresetId !== CLE_API_PROVIDER_ID && (
                      <div className="oauth-link">
                        <label>
                          {t(
                            "codex.modelProviders.newProviderName",
          ...
          {deleteConfirm && (
            <ModalPortal>
              ...
              <div
                ...
                role="alertdialog"
                aria-modal="true"
                aria-labelledby="codex-delete-confirm-title"
                onClick={(e) => e.stopPropagation()}
              >
                <div className="modal-header">
                  <h2 id="codex-delete-confirm-title">{t("common.confirm")}</h2>
                  <button
                    className="modal-close"
