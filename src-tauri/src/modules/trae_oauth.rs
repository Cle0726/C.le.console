...
<head>
  ...
  <title>Trae OAuth - C.le.控制台</title>
  <style>
    :root {{
      color-scheme: dark;
      --bg: #0f172a;
...
<body>
  <main class="card">
    <div class="brand"><span class="mark">✓</span><span>C.le.控制台</span></div>
    <div class="status {tone}">{badge}</div>
    <h1>{title}</h1>
    <p id="hint">{message}</p>
    <div class="foot">完成后可以关闭此页面，回到 C.le.控制台 继续操作。</div>
  </main>
  {script}
</body>
</html>"#,
...
async fn request_login_guidance(
    ...
    for endpoint in endpoints {
        ...
        let request = client
            ...
            .header("User-Agent", "Trae/1.0.0 antigravity-cle-console")
            .json(&body);
